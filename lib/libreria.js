...
module.exports = {
    ...
    // Devuelve la fecha calendario local AR como 'YYYY-MM-DD' (para columnas DATE)
    FechaLocalASqlDate: function(fecha) {
        if (!(fecha instanceof Date)) return '';
        const local = toZonedTime(fecha, ZONA_AR);
        const year = local.getFullYear();
        const month = String(local.getMonth() + 1).padStart(2, '0');
        const day = String(local.getDate()).padStart(2, '0');
        return `${year}-${month}-${day}`;
    },
    // Devuelve la fecha calendario en UTC como 'YYYY-MM-DD' (para columnas DATE)
    FechaUtcASqlDate: function(fecha) {
        if (!(fecha instanceof Date)) return '';
        const year = fecha.getUTCFullYear();
    ...
    ExtraerHora: function (fecha) {
        if (!fecha) return '';
        // Convertir desde UTC a horario local de AR de manera consistente
        const localFecha = toZonedTime(fecha, ZONA_AR);
